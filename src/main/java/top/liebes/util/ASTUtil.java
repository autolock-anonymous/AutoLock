...
//                System.out.println(info2.getName() + " : " + System.identityHashCode(info2.getName()));
    ...
    public static MethodDeclaration getMethodDeclaration(String[] modifiers, String returnType, String name, String[] params, String body) {
        ASTParser parser = ASTParser.newParser(Env.JAVA_VERSION);
        parser.setKind(ASTParser.K_CLASS_BODY_DECLARATIONS);
        StringBuilder source = new StringBuilder();
        source.append(String.join(" ", modifiers))
                .append(" ").append(returnType).append(" ")
                .append(name).append("(").append(String.join(",", params))
                .append("){").append(body).append("}");
        parser.setSource(source.toString().toCharArray());
        TypeDeclaration typeDeclaration = (TypeDeclaration) parser.createAST(null);
        return (MethodDeclaration) typeDeclaration.bodyDeclarations().get(0);
    }
