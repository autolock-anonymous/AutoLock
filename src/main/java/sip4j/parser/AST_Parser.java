	...
	public static  E_Class createNewClass(ITypeBinding node){
		...
		if(tempClass != null){
			...
			if(node.isAnonymous()){
				...
				_class.setClassQName(node.getDeclaringClass().getQualifiedName());
			}
			else{
				_class.setName(node.getName());
			...
			if(node.getSuperclass() != null){
				...
				if (superClass != null){
					if(superClass.isFromSource()){
						...
						_superClass.setName(superClass.getName());
	...
	public static IMethodBinding getMethodInvocBinding(Expression exp){
		...
		if(exp!=null){
			if (exp.getNodeType() == ASTNode.CLASS_INSTANCE_CREATION) {
				...
				if (f.resolveMethodBinding() != null) {
						...
						if (arg.isField() == true) {
							...
							if(sourceMethod == null){
								...
								if(mDecl.resolveBinding().getDeclaringClass() != null && mDecl.resolveBinding().getDeclaringClass().isInterface() == false){
									...
									AST_Parser.addClassToPackage(Data_Generator.getPackage(),_parentClass);
							...
							if(sourceMethod == null ){
								if(mDecl!=null){
									...
									if(mDecl.resolveBinding().getDeclaringClass() != null && mDecl.resolveBinding().getDeclaringClass().isInterface()==false){
										...
										AST_Parser.addClassToPackage(Data_Generator.getPackage(),parentClass);
...
//AST.resolveWellKnownType(String)*/
	...
	public static void addMethodInvocationExp(MethodInvocation thisMethodInv){
		...
		if(thisMethodInv != null){
			...
			List<Expression> argList = AST_Parser.getArguments(thisMethodInv);
			// add method invocation expression at class and method level
			//  should pass parameter list here to map parameters with arguments
			AST_Parser.addMethodInvocations(imb, argList, qualObj, mDecl);
		}
	}
 ...
 for(E_Class c: _class){
	...
	public static void addClassMethod(E_Class _class, E_Method _method){
		if (_class != null && _method!=null) {
			LinkedList<E_Method> methodList = _class.getMethods();
			if (methodList != null) {
				// Checking if the same method already exists in the class
				if (! ifMethodExists(methodList, _method)) {
					_class.addMethod(_method);
				}
			}
		}
	}
	public static void addClassToPackage(E_Package pkg, E_Class _class){
		if (pkg != null && _class!= null) {
			LinkedList<E_Class> classList = pkg.getClasses();
			if (classList != null) {
				// Checking if the same method already exists in the class
				if (! ifClassExists(classList,_class)) {
 ...
 */
	...
	public static boolean ifClassExists(LinkedList<E_Class> list,
										E_Class _class) {
		boolean flag = false;
		for(E_Class data : list){
			if (data.getName().equals(_class.getName())
					&& data.getClassQName().equals(_class.getClassQName())){
				return true;
			}
		}
		return false;
...
*/
	...
	public static void checkRightSide(Expression laExp,Expression raExp, E_Method _method){
		...
		if(raExp != null){
			...
			else if((raExp.resolveTypeBinding() != null && raExp.resolveTypeBinding().isPrimitive()) || raExp.getNodeType() == ASTNode.NUMBER_LITERAL){
				IVariableBinding leftExpB = AST_Parser.getVariableBinding(laExp);
				AST_Parser.addFieldAccessExp(_method,leftExpB,GlobalVariables.WRITE,expType,AST_Parser.ifReturnedField(laExp), qualObj);
			}
			else{
...
//				System.out.println("PMethod = "+pMethod.getName());
	...
	@SuppressWarnings("null")
	public static List<MethodInvocation> getMethodInvocations(final TypeDeclaration node){
		List<MethodInvocation> tempInvokedMethods = new ArrayList<>();
		List<MethodInvocation> invokedMethods = new ArrayList<>();
	...
	public static List<MethodInvocation> getThisMethodInvokation(final MethodDeclaration snode){
		List<MethodInvocation> invokedMethod = new LinkedList<>();
		List<MethodInvocation> tempp = new LinkedList<>();
	...
	public static List<ClassInstanceCreation> getConstructorInvocation(final MethodDeclaration snode){
		...
		// this code solves the problem of unreachability of java element. this happens when the project is not running on eclipse environment
		if(binding.getJavaElement() == null){
			String folder = Env.SOURCE_FOLDER;
			File root = new File(folder);
			List<File> files = FileUtil.getFiles(root, new String[]{"java"});
			...
			public boolean visit(ClassInstanceCreation node) {
				...
				if(cmb!=null){
					...
					if (ifUserDefinedMethod(cmb)){
						...
						if(qualObj.getName().equals("this")){
							qualObj.setName(cmb.getDeclaringClass().getQualifiedName());
						}
						// fetch this method arguments
	...
	public static List<MethodInvocation> getThisMethodCall(CompilationUnit cu, final IMethodBinding smb, List<MethodInvocation> methodCall ){
		...
		final List<MethodInvocation> invokedMethods = new ArrayList<>();
		cu.accept(new ASTVisitor(){
			@Override
			public boolean visit(MethodInvocation node) {
				IMethodBinding tmb = node.resolveMethodBinding();
				if(tmb!=null){
					if (ifUserDefinedMethod(tmb)){
						...
						if (tmb.getName().equals(smb.getName())
								&& tmb.getDeclaringClass().getQualifiedName().equals(smb.getDeclaringClass().getQualifiedName())) {
							invokedMethods.add(node);
	...
	public static List<ClassInstanceCreation> getThisConsCall(CompilationUnit cu, final IMethodBinding smb, List<ClassInstanceCreation> methodCall ){
		//final MethodInvocation[] methodInvocation = new MethodInvocation[1];
		final List<ClassInstanceCreation> invokedMethods = new ArrayList<>();
		cu.accept(new ASTVisitor(){
			@Override
			public boolean visit(ClassInstanceCreation node) {
			...
			mDec.getBody().accept(new ASTVisitor() {
				...
				public boolean visit(SuperMethodInvocation node) {
					...
					if(smb != null){
						...
						// if it is a user defined super constructor
						if(AST_Parser.ifUserDefinedMethod(smb)){
							List<Expression> argList = new LinkedList<>();
							if(node.arguments() != null){
								for(int i = 0;i < node.arguments().size();i++) {
									argList.add((Expression) node.arguments().get(i));
...
//return methodCalls;
	...
	public static void addMethodData(MethodDeclaration node, E_Object obj){
		...
		if(node.resolveBinding().getDeclaringClass() != null && node.resolveBinding().getDeclaringClass().isInterface()==false){
			...
			AST_Parser.addClassToPackage(pkg,_class);
		}
		//E_Class _class = Data_Controller.fetchLastClass();
